...
void build(int node,int start,int end)
{
	...
	Tree[node]=(Tree[2*node+1] < Tree[2*node+2])?(Tree[2*node+1]):(Tree[2*node+2]);     //time complexity T(n/2)+T(n/2) + O(1) - O(n)
	return;
}
...
int main()
{
  ...
  while(t--){
    int i,j;
    cin>>c>>i>>j;
    if(c=='q')cout<<query(0,0,n-1,i-1,j-1)<<endl;
    else update(0,0,n-1,i-1,j-1);
  }
}
